...
export function buildSystemPrompt(suttas: SuttaResult[], userMessage = ""): string {
  const suttaText = suttas
    ...
    .join("\n\n")
  if (!suttaText) {
    // ค้นไม่พบสูตรที่ตรงคำถาม — บอกโมเดลตรงๆ ห้ามให้มันเดาอ้างอิงเอง หรือแต่งเนื้อหา/โครงสร้างธรรมะเอง
    return `${VERIFIER_PROMPT}\n\n--- หมายเหตุการค้นหา / Retrieval note ---\nรอบนี้ระบบค้นไม่พบพระสูตรที่ตรงกับคำถามโดยเฉพาะ ให้ตอบจากความรู้ทั่วไปเกี่ยวกับพระพุทธศาสนาเฉพาะส่วนที่มั่นใจจริงๆ เท่านั้น ห้ามระบุชื่อหรือเลขพระสูตรที่ไม่แน่ใจเด็ดขาด และห้ามแต่งรายละเอียด นิยาม การแบ่งประเภท หรือโครงสร้างคำสอนขึ้นเองสำหรับคำศัพท์บาลี/ธรรมะที่เฉพาะเจาะจงหรือไม่คุ้นเคย — ถ้าไม่แน่ใจในความหมายหรือรายละเอียดของคำนั้นจริงๆ ให้บอกตรงๆ ว่าไม่แน่ใจ/ไม่พบหลักฐานชัดเจน ดีกว่าเดาแล้วอาจผิด\nNo specific sutta was retrieved for this question. Answer only from general Buddhist knowledge you are genuinely confident about. Never state a sutta name or number you are not certain of, and never invent definitions, categorizations, or doctrinal structure for specific or unfamiliar Pali/Buddhist terms — if you are not sure of a term's precise meaning or details, say so plainly instead of guessing.${languageOverride(userMessage)}`
  }
